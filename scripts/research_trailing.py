#!/usr/bin/env python3
"""
Trailing Research Script - Forensic Dataset Extraction (V3 - High Fidelity)
Extracts advanced features and outcomes for trades that reached 1.3R.
Includes slope, RSI, and MACD for better prediction.
"""
import sys
import os
import pandas as pd
...
logger = logging.getLogger("RESEARCH_TRAILING_V3")
...
def calculate_indicators(df):
    ...
    df['ema_200_slope'] = df['ema_200'].diff(3)
...
def run_extraction_for_symbol(mt5, symbol):
    logger.info(f"📊 Deep Extraction for {symbol}...")
    ...
    for i in range(200, len(df) - 50):
        ...
        tp_final = entry_price + (sl_dist * FINAL_TP_R) if sig == 1 else entry_price - (sl_dist * FINAL_TP_R)
        
        reached_partial = False
        outcome = 0 
        
        # Capture features at signal for potential future use
        ...
        for j in range(i + 1, len(df)):
            ...
            if not reached_partial:
                if (sig == 1 and high >= tp_partial) or (sig == -1 and low <= tp_partial):
                    ...
                    # EXTRACT ADVANCED FEATURES AT PARTIAL MOMENT
                    features = {
                        'symbol': symbol,
                        ...
                        'ema_200_slope': m_row['ema_200_slope'] / atr,
                        'dist_ema200': abs(m_row['close'] - m_row['ema_200']) / atr,
                        'time_to_partial': (m_row['date'] - entry_time).total_seconds() / 3600.0,
                    }
                    max_excursion = 0.0
                ...
                # TRACK MAXIMUM EXCURSION AFTER PARTIAL
                current_exc = (high - entry_price) / sl_dist if sig == 1 else (entry_price - low) / sl_dist
                max_excursion = max(max_excursion, current_exc)
                
                sl = entry_price
                if (sig == 1 and low <= sl) or (sig == -1 and high >= sl):
                    outcome = 0
                    break
                if (sig == 1 and high >= tp_final) or (sig == -1 and low <= tp_final):
                    outcome = 1
                    break
                    
        if reached_partial:
            features['max_excursion_r'] = max_excursion
...
def main():
    ...
    if all_data:
        ...
        out_path = "data/research/trailing_dataset_v3.csv"
        res_df.to_csv(out_path, index=False)
        logger.info(f"✅ Extraction Complete: {len(res_df)} samples saved.")
    else:
